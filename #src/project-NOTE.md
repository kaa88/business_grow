...
### TO DO:
- modal access call msg настроить отступы на декстопе, сделать полоску как элемент грида, чтобы всегда была посередине
- поворот экрана - сделать чтобы в любой ориентации на мобиле был моб вид (проверка сразу и ширины и высоты)
